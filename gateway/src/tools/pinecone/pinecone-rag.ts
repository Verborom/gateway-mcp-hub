...
let PINECONE_API_KEY: string | undefined;
let PINECONE_HOST: string | undefined;
...
function initClients() {
  if (!pinecone) {
    ...
    PINECONE_HOST = process.env.PINECONE_HOST || 'https://gateway-4k3euk3.svc.gcp-us-central1-4a9f.pinecone.io';
    OPENAI_API_KEY = process.env.OPENAI_API_KEY;
    
    if (!PINECONE_API_KEY || !OPENAI_API_KEY) {
      console.error('[Pinecone] Warning: API keys not loaded yet');
      return false;
    }
    ...
    index = pinecone.index('gateway', PINECONE_HOST);
  }
  return true;
}
...
export async function handlePineconeTool(name: string, args: any): Promise<string> {
  if (!initClients()) {
    throw new Error('API keys not initialized - please check environment variables');
  }
...
async function syncWithPinecone(tokenLimit: number) {
  if (!initClients() || !index) {
    console.error('[Background] Cannot sync - API keys not initialized');
    return { totalTokens: 0, messageCount: 0, content: '' };
  }
